...
void solve_subtree(int row, struct wordle *w,  struct node *dict, 
    ...
    while (temp != NULL) {
        ...
        if(match_constraints(temp->word, parent->con, w, row) == 1){
            ...
            printf("problem?\n");
            struct solver_node *curr = malloc(sizeof(struct solver_node));
            ...
            // recursions
            if (child_node != NULL){solve_subtree(row+1, w, dict, child_node);}     
        }
    ...
    // printf("node end\n");
    // recursions
    // if (parent->child_list != NULL){solve_subtree(row+1, w, temp, parent->child_list);}
    // if (parent->next_sibling != NULL){ solve_subtree(row+1, w, temp, parent->next_sibling);}
